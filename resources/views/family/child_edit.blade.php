  ...
  <div class="bg-white shadow p-4 rounded-[12px] mt-6 p-8">
    <form action="{{ route('family.child.update', $child) }}" method="POST">
      ...
      <div class="mb-5">
        ...
        <select name="relation" class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
      ...
      <div class="mb-5">
        ...
        <input type="text"
               name="business_name"
               value="{{ old('business_name', $child->business_name) }}"
               class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-2 focus:ring-blue-500">
      </div>
      ...
      <div class="mb-5">
        ...
        <select name="blood_group" class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
          @foreach(['A+','A-','B+','B-','O+','O-','AB+','AB-','જાણ નથી'] as $bg)
            <option value="{{ $bg }}" {{ old('blood_group', $child->blood_group)==$bg?'selected':'' }}>
              {{ $bg }}
            </option>
          @endforeach
        </select>
          ...
          @foreach(['પુરુષ','સ્ત્રી','અન્ય'] as $g)
            <label class="flex items-center">
              <input type="radio"
                     name="gender"
                     value="{{ $g }}"
                     {{ old('gender', $child->gender)==$g?'checked':'' }}
      ...
      <div class="mb-5">
        ...
        <select name="marital_status" class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
          @foreach(['પરિણીત','અપરિણીત','ગંગા સ્વરૂપ','વિધુર','છૂટાછેડા','સગાઈ'] as $ms)
            <option value="{{ $ms }}" {{ old('marital_status', $child->marital_status)==$ms?'selected':'' }}>
              {{ $ms }}
            </option>
          @endforeach
        </select>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <input type="text"
                 name="mosal_name"
                 value="{{ old('mosal_name', $child->mosal_name) }}"
                 class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-2 focus:ring-blue-500">
        </div>
        <div>
          ...
          <input type="text"
                 name="mosal_branch"
                 value="{{ old('mosal_branch', $child->mosal_branch) }}"
                 class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-2 focus:ring-blue-500">
        </div>
        <div>
