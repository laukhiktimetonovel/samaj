...
<div class="py-4 md:py-6 w-full px-4 md:min-h-screen md:max-h-screen md:overflow-y-auto">
  ...
  <div class="bg-white shadow p-8 rounded-[12px] mt-6">
    ...
    <div class="mb-5">
        ...
        @error('photo') <div class="text-red-500 mt-1">{{ $message }}</div> @enderror
    </div>
      {{-- Full Name --}}
      <div class="mb-5">
      ...
      <div class="mb-5">
        ...
        <input type="text" name="mobile" value="{{ old('mobile', $parent->mobile) }}"
               class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
        @error('mobile') <div class="text-red-500 mt-1">{{ $message }}</div> @enderror
      </div>
      ...
      {{-- Village Name --}}
        <div class="mb-5">
            ...
            @foreach($gamOptions as $opt)
                <option value="{{ $opt }}" {{$opt == $parent->village_name ? 'selected' : ''}}>{{ $opt }}</option>
            @endforeach
      ...
      <div class="mb-5">
        ...
        <input type="text" name="current_address" value="{{ old('current_address', $parent->current_address) }}"
               class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
      </div>
      ...
      <div class="mb-5">
        ...
        <input type="text" name="village_address" value="{{ old('village_address', $parent->village_address) }}"
               class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
      </div>
      ...
      <div class="mb-5">
        ...
        <input type="text" name="business_address" value="{{ old('business_address', $parent->business_address) }}"
               class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
      </div>
      ...
      <div class="mb-5">
        ...
        <select name="blood_group" class="w-full border border-gray-300 rounded-md px-4 py-2">
          @foreach(['A+','A-','B+','B-','O+','O-','AB+','AB-','જાણ નથી'] as $bg)
            <option value="{{ $bg }}" {{ old('blood_group', $parent->blood_group)==$bg?'selected':'' }}>
              {{ $bg }}
            </option>
          @endforeach
        </select>
      ...
      <div class="mb-5">
        ...
        <select name="marital_status" class="w-full border border-gray-300 rounded-md px-4 py-2">
          @foreach(['પરિણીત','અપરિણીત','ગંગા સ્વરૂપ','વિધુર','છૂટાછેડા','સગાઈ'] as $ms)
            <option value="{{ $ms }}" {{ old('marital_status',$parent->marital_status)==$ms?'selected':'' }}>
              {{ $ms }}
            </option>
          @endforeach
        </select>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          ...
          <input type="text" name="mosal_branch" value="{{ old('mosal_branch',$parent->mosal_branch) }}"
                 class="w-full border rounded-md px-4 py-2 focus:ring-2 focus:ring-blue-500">
        </div>
        <div>
          <label class="block text-[16px] font-medium text-[#B3541E] mb-1">મોસાળના ગામ:</label>
          <input type="text" name="mosal_village" value="{{ old('mosal_village',$parent->mosal_village) }}"
                 class="w-full border rounded-md px-4 py-2 focus:ring-2 focus:ring-blue-500">
        </div>
      </div>
